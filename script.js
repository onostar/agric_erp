...
// prinit receipt for customer investment payment
function printInvestmentReceipt(invoice){
     window.open("../controller/investment_receipt.php?receipt="+invoice)
     return false;
...
function updateField(){
 ...
 // update customer land  details
function updateCustomerLand(){
     let assigned_id = document.getElementById("assigned_id").value;
     let field_size = document.getElementById("field_size").value;
     let latitude = document.getElementById("latitude").value;
     let longitude = document.getElementById("longitude").value;
    
    if(!field_size || parseFloat(field_size) <= 0){
          alert("Please input field size!");
          $("#field_size").focus();
          return;
     }else if(!latitude){
          ...
          $.ajax({
               type : "POST",
               url : "../controller/update_customer_field.php",
               data : {assigned_id:assigned_id, field_size:field_size,  latitude:latitude, longitude:longitude,},
               beforeSend: function(){
                    $("#farm_fields").html("<div class='processing'><div class='loader'></div></div>");
               },
               success : function(response){
...
function postInvestment(){
     ...
     if(payment_mode.length == 0 || payment_mode.replace(/^\s+|\s+$/g, "").length == 0){
          ...
          if(confirmPost){
               $.ajax({
                    type : "POST",
                    url : "../controller/pay_investment.php",
                    data : {posted:posted, customer:customer, investment:investment, payment_mode:payment_mode, amount:amount, details:details, store:store, invoice:invoice, bank:bank, trans_date:trans_date, balance:balance},
                    beforeSend : function(){
                         $("#fund_account").html("<div class='processing'><div class='loader'></div></div>");
                    },
                    success : function(response){
...
function payReturn(){
     ...
     if(payment_mode.length == 0 || payment_mode.replace(/^\s+|\s+$/g, "").length == 0){
          ...
          if(confirmPost){
               $.ajax({
                    type : "POST",
                    url : "../controller/pay_returns.php",
                    data : {posted:posted, customer:customer, schedule:schedule, payment_mode:payment_mode, amount:amount, details:details, store:store, invoice:invoice, bank:bank, trans_date:trans_date},
                    beforeSend : function(){
                         $("#fund_account").html("<div class='processing'><div class='loader'></div></div>");
                    },
