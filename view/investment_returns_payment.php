...
<?php
    ...
    if(isset($_GET['customer']) && isset($_GET['schedule'])){
        ...
        $receipt_id = "RET".$todays_date.$ran_num.$user_id.$schedule;
        ...
        //get loan details
        $lns = $get_details->fetch_details_cond('investment_returns', 'schedule_id', $schedule);
        foreach($lns as $lns){
            $investment = $lns->investment_id;
            $amount_due = $lns->amount_due;
            $payment_status = $lns->payment_status;
        }
       //get total paid
       $ttls = $get_details->fetch_sum_single('investment_returns', 'amount_paid', 'schedule_id', $schedule);
       ...
       //get total due
       /*$ttlx = $get_details->fetch_sum_single('investment_returns', 'amount_due', 'investment_id', $investment);
       if(gettype($ttlx) == 'array'){
        ...
        $invs = $get_details->fetch_details_cond('investments', 'investment_id', $investment);
        foreach($invs as $inv){
...
<div class="back_invoice">
    <button class="page_navs" id="back" onclick="showPage('pay_returns.php?customer=<?php echo $customer_id?>&investment=<?php echo $investment?>')"><i class="fas fa-angle-double-left"></i> Back</button>
...
<div id="deposit" class="displays">
    ...
    <div class="fund_account" style="width:80%; margin:5px 0;">
        <h3 style="background:var(--labColor); text-align:left">Post Investor Concentrate Returns</h3>
            ...
            <section class="addUserForm">
                <div class="inputs" style="flex-wrap:wrap">
                    ...
                    <div class="data" style="width:100%; margin:5px 0">
                        ...
                        <textarea name="details" id="details" cols="30" rows="5" >Return on Investment</textarea>
                    </div>
                    <div class="data" style="width:50%; margin:5px 0">
                        <button type="submit" id="post_exp" name="post_exp" onclick="payReturn('fund_account')">Post payment <i class="fas fa-cash-register"></i></button>
                    </div>
                </div>
            </section>
