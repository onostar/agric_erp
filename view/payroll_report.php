...
?>
   <style>
            ...
            <div class="from_to_date" style="width:auto; margin:0!important; padding:0">
                ...
                <select name="payroll_month" id="payroll_month" onchange="getMonthlyPayrolll(this.value)">
                    ...
                    <?php
                        ...
                        $results = $get_role->fetch_payroll_months($store);
                        if(!empty($results)){
                            foreach($results as $result){
                    ?>
                    <option value="<?php echo date("Y-m-d", strtotime($result->payroll_date))?>"><?php echo date("F, Y", strtotime($result->payroll_date))?></option>
                    <?php }}?>
                </select>
            </div>
    ...
    <h2>Payroll Report for <?php echo date("F, Y")?></h2>
    <hr>
    <div class="search">
    ...
    <table id="item_list_table" class="searchTable">
        ...
        <?php
                ...
                $details = $get_items->fetch_details_curmonthCon('payroll', 'payroll_date', 'store', $store);
