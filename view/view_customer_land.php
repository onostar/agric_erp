    ...
    $rows = $get_item->fetch_details_cond('assigned_fields', 'assigned_id', $id);
     if(gettype($rows) == 'array'){
        foreach($rows as $row):
            ...
            //get land details
            $lnds = $get_item->fetch_details_cond('fields', 'field_id', $row->field);
            foreach($lnds as $lnd){
                $field_name = $lnd->field_name;
                
            }
            
    ?>
    <div class="add_user_form priceForm" style="margin:10px 50px;">
        <h3 style="background:var(--tertiaryColor)">UPDATE "<?php echo strtoupper($field_name)?>" DETAILS FOR <?php echo $customer?></h3>
        <form style="text-align:left;">
            <div class="inputs" style="flex-wrap:wrap; gap:1rem; justify-content:left">
                <!-- <div class="data item_head"> -->
                    <input type="hidden" name="assigned_id" id="assigned_id" value="<?php echo $id?>" required>
               ...
               <!-- <div class="data" style="width:31%">
                ...
                <div class="data" style="width:auto">
                    <button type="button" id="change_price" name="change_price" onclick="updateCustomerLand()">Update <i class="fas fa-save"></i></button>
                    <a href="javascript:void(0)" title="close form" style='background:red; padding:10px; border-radius:15px; border:1px solid #fff;box-shadow:1px 1px 1px #222; color:#fff' onclick="showPage('customer_lands.php')">Return <i class='fas fa-angle-double-left'></i></a>
                </div>
