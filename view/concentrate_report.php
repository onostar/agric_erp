...
<div id="production_report" class="displays management" style="width:100%!important;">
    <div class="select_date">
        ...
        <section>    
            ...
            <button type="submit" name="search_date" id="search_date" onclick="search('search_concentrate_production.php')">Search <i class="fas fa-search"></i></button>
        </section>
    </div>
    <div class="displays allResults new_data" id="revenue_report">
        <h2>Today's Concentrate Production from <?php echo $store_name?></h2>
        <hr>
        <div class="search">
        ...
        <table id="data_table" class="searchTable">
            ...
            <tbody>
                <?php
                    ...
                    $details = $get_users->fetch_details_curdateCon('concentrate_production', 'date(date_produced)', 'store', $store);
        ...
        <?php
            ...
            //total raw material
            $prds = $get_users->fetch_sum_curdateCon('concentrate_production', 'total_pineapple_cost', 'date_produced', 'store', $store);
            foreach($prds as $prd){
                $pineapple_cost = $prd->total;

            }
            //total pineapple crown
            $crws =   $prds = $get_users->fetch_sum_curdateCon('concentrate_production', 'total_crown_value', 'date_produced', 'store', $store);
            foreach($crws as $crw){
                $crown_value = $crw->total;
            }
            //total pineapple peels
            $crws =   $prds = $get_users->fetch_sum_curdateCon('concentrate_production', 'total_peel_value', 'date_produced', 'store', $store);
            foreach($crws as $crw){
