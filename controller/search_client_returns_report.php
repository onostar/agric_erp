<?php
    ...
    $details = $get_revenue->fetch_details_2dateConOrder('return_payments', 'customer', 'date(post_date)', $from, $to, $customer, 'post_date');
    $n = 1;
?>
<h2>Investment Returns Received between '<?php echo date("jS M, Y", strtotime($from)) . "' and '" . date("jS M, Y", strtotime($to))?>'</h2>
    <hr>
    <div class="search">
        <input type="search" id="searchRevenue" placeholder="Enter keyword" onkeyup="searchData(this.value)">
        <a class="download_excel" href="javascript:void(0)" onclick="convertToExcel('data_table', 'Investment Returns Received')"title="Download to excel"><i class="fas fa-file-excel"></i></a>
    </div>
...
<?php
    ...
    $ttls = $get_revenue->fetch_sum_2dateCond('return_payments', 'amount_in_dollar', 'customer', 'date(post_date)', $from, $to,  $customer);
